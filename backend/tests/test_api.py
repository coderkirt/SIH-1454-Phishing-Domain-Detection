...
def test_stats_daily_summary():
    ...
    assert "average_response_time_ms" not in data


def test_analyze_content_multi_link():
    response = client.post("/api/v1/analyze/content", json={
        "source_type": "whatsapp",
        "text": (
            "URGENT! Your SBI account will be blocked today. "
            "Complete KYC immediately: https://sbi-secure-example.invalid "
            "Backup: https://verify-kyc.invalid Enter OTP"
        ),
    })
    assert response.status_code == 200
    data = response.json()
    ...
    assert data["privacy"]["raw_message_stored"] is False
    assert "export" in data
    assert "raw_text" not in data["export"]


def test_analyze_requires_input():
    response = client.post("/api/v1/analyze/content", json={"source_type": "text", "text": ""})
    assert response.status_code == 400


def test_report_requires_auth():
    response = client.post("/api/v1/report", json={"url": "https://evil.example", "label": "scam"})
...
def test_screenshot_upload_with_hint_url():
    png = bytes.fromhex(
    ...
    response = client.post(
        ...
        data={"hint": "https://paypa1.com"},
    )
    assert response.status_code == 200
    data = response.json()
    assert data["source_type"] == "screenshot"
    assert any("paypa1.com" in (link.get("url") or "") for link in data.get("links") or [])


def test_screenshot_upload_without_signal():
    png = bytes.fromhex(
        "89504e470d0a1a0a0000000d49484452000000010000000108060000001f15c489"
        "0000000a49444154789c63000100000500010d0a2db40000000049454e44ae426082"
    )
