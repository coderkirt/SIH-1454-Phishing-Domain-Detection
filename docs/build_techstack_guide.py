# -*- coding: utf-8 -*-
"""Build PhishShield Tech Stack & Team Learning Guide (MD + XLSX + HTML + PDF)."""
...
MD_PATH = ROOT / "PhishShield_TechStack_Team_Learning_Guide.md"
XLSX_PATH = ROOT / "PhishShield_TechStack_Inventory.xlsx"
HTML_PATH = ROOT / "docs" / "_techstack_guide.html"
PDF_PATH = ROOT / "PhishShield_TechStack_Team_Learning_Guide.pdf"
CHROME = r"C:\Program Files\Google\Chrome\Application\chrome.exe"


INVENTORY = [
    # Category, Technology, Purpose, Files, Status, Priority, Team Role, Learning Topics, Viva Topics
    ("Frontend", "React 19.2.8", "Dashboard UI: Scanner, History, Reports, Statistics, auth pages", "frontend/src/App.jsx, pages/, components/", "Implemented", "HIGH", "Frontend Developer", "Components, props, state, hooks, JSX", "Why React; Virtual DOM; component tree"),
    ("Frontend", "React Router DOM 7", "Client-side routes and ProtectedRoute for dashboard pages", "frontend/src/App.jsx, components/ProtectedRoute.jsx", "Implemented", "HIGH", "Frontend Developer", "Routes, Navigate, nested layouts", "SPA routing vs server routing"),
    ("Frontend", "Axios", "HTTP client to FastAPI; attaches JWT from localStorage", "frontend/src/services/api.js", "Implemented", "HIGH", "Frontend Developer", "REST, interceptors, error handling", "How frontend talks to backend"),
    ("Frontend", "Lucide React", "Icons in layouts and pages", "frontend/src/layouts/, pages/", "Implemented", "LOW", "Frontend Developer", "Icon components", "Not a core viva topic"),
    ("Frontend", "Recharts", "Charts on Statistics / Dashboard", "frontend/src/pages/Statistics.jsx, Dashboard.jsx", "Implemented", "MEDIUM", "Frontend Developer", "Bar/line charts from API JSON", "How dashboard numbers are fetched"),
    ("Frontend", "Vite 8", "Dev server on 127.0.0.1:5173 and production bundler", "frontend/vite.config.js, package.json", "Implemented", "MEDIUM", "Frontend Developer", "npm run dev/build, env VITE_API_URL", "Dev vs production build"),
    ("Frontend", "Tailwind CSS 4 (@tailwindcss/vite)", "Utility CSS styling", "frontend/src (className), vite.config.js", "Implemented", "MEDIUM", "Frontend Developer", "Utility classes, responsive layout", "Why Tailwind vs CSS files"),
    ("Frontend", "oxlint", "JS/JSX lint (npm run lint)", "frontend/.oxlintrc.json", "Implemented", "LOW", "Frontend Developer", "Run lint before commit", "Quality tools"),
    ("Backend", "Python 3", "Language of FastAPI services and tests", "backend/app/**/*.py, backend/tests/", "Implemented", "HIGH", "Backend Developer", "Functions, modules, typing, exceptions", "Why Python for this project"),
    ("Backend", "FastAPI", "REST API: analyze, threat, user, stats, scans, reports", "backend/app/main.py, routes/", "Implemented", "HIGH", "Backend Developer", "Routers, Pydantic, Depends, status codes", "Why FastAPI not Flask; OpenAPI"),
    ("Backend", "Uvicorn", "ASGI server, host 0.0.0.0 port 8000", "backend/app/main.py, README.md", "Implemented", "HIGH", "Backend Developer", "Run uvicorn, host/port, reload", "ASGI vs WSGI"),
    ("Backend", "Pydantic v2", "Request body validation (ContentRequest, UserRegister, etc.)", "backend/app/routes/*.py", "Implemented", "HIGH", "Backend Developer", "BaseModel, validation errors", "Input validation vs SQL injection"),
    ("Backend", "pydantic-settings", "Settings object (SECRET_KEY, DATABASE_URL string)", "backend/app/config.py", "Partial", "MEDIUM", "Backend Developer", "BaseSettings, env files", "config.py DATABASE_URL is unused by sqlite3 connection"),
    ("Backend", "python-dotenv", "Load backend/.env (SECRET_KEY, GSB key)", "backend/app/services/auth.py, external_intel.py", "Implemented", "HIGH", "Backend / Security", "Never commit .env", "Secrets handling"),
    ("Backend", "requests", "Outbound HTTP: Safe Browsing, OpenPhish, RDAP, redirects", "backend/app/services/external_intel.py, redirects.py, ad_signals.py", "Implemented", "HIGH", "Backend Developer", "Timeouts, fail-soft", "What if intel APIs are down"),
    ("Backend", "httpx", "Listed for tests / HTTP client", "backend/requirements.txt, tests/", "Implemented", "MEDIUM", "Backend Developer", "TestClient vs httpx", "Testing APIs"),
    ("Backend", "python-multipart", "QR/screenshot file uploads", "backend/app/routes/analyze.py", "Implemented", "MEDIUM", "Backend Developer", "UploadFile, size cap 5 MB", "File upload security"),
    ("Backend", "SQLAlchemy 2", "ORM model files exist (User, UrlCheck, Threat)", "backend/app/models/, requirements.txt", "Dependency present, usage not confirmed in live routes", "LOW", "Database Developer", "ORM vs raw SQL", "Jury: models exist but routes use sqlite3"),
    ("Database", "SQLite (sqlite3)", "Live persistence: users, url_checks, content_scans, reports", "backend/app/database/connection.py, backend/threats.db", "Implemented", "HIGH", "Database Developer", "Tables, PK/FK, PRAGMA foreign_keys, parameterized SQL", "Why SQLite now; PostgreSQL later"),
    ("Database", "PostgreSQL", "Not connected. config.py default is sqlite URL, not Postgres", "backend/app/config.py comment only", "Planned", "MEDIUM", "Database Developer", "Postgres vs SQLite, migrations", "Do not claim live Postgres"),
    ("Database", "Alembic", "No alembic.ini or migrations folder", "not in repo", "Not implemented", "LOW", "Database Developer", "Schema migrations", "How schema is created today: init_db()"),
    ("AI / Fusion", "Weighted signal fusion (not a trained model)", "Combines url/domain/ml/nlp/psych/intel/brand/redirect/sender/community", "backend/app/services/risk_engine.py", "Implemented", "HIGH", "ML Developer", "Weights, caps, levels, confidence formula", "ml key copies URL risk; not sklearn"),
    ("AI / Fusion", "Rule-based URL checker", "Heuristics: TLD, brand typos, shorteners, keywords, trusted list", "backend/app/services/url_checker.py", "Implemented", "HIGH", "ML / Backend", "Heuristics vs ML, allowlists", "False positives on new domains"),
    ("AI / Fusion", "scikit-learn / TensorFlow / Keras CNN", "No .pkl/.h5, no train script, no dataset in repo", "absent", "Not implemented", "LOW", "ML Developer", "What a real model would need", "Do not claim CNN"),
    ("NLP", "Keyword / substring psychological detector", "Urgency, fear, OTP, countdown, greed, secrecy, Hinglish phrases", "backend/app/services/psychology.py", "Implemented", "HIGH", "ML / Security Lead", "Pattern lists, score cap, psych-only guard", "Not spaCy/BERT; substring matching"),
    ("NLP", "Language heuristic", "Devanagari count + Hinglish tokens → en / hi / en-hi", "backend/app/services/content_normalizer.py", "Implemented", "MEDIUM", "ML Developer", "Heuristic vs real NLP", "How language is detected"),
    ("NLP", "spaCy / transformers / BERT", "Not in requirements or imports", "absent", "Not implemented", "LOW", "ML Developer", "Future NLP", "Do not claim transformer NLP"),
    ("Vision", "OpenCV QRCodeDetector + NumPy", "Decode QR URLs from uploaded images; does not visit destination", "backend/app/services/qr_analyzer.py", "Implemented", "HIGH", "ML / Backend", "imdecode, QRCodeDetector", "QR is decode-then-analyze-URL"),
    ("Vision", "Pillow", "Open screenshot bytes if OCR path runs", "backend/app/routes/analyze.py", "Implemented", "MEDIUM", "Backend Developer", "PIL Image.open", "Used with optional Tesseract"),
    ("Vision", "Tesseract / pytesseract", "Optional OCR on screenshots; not in requirements.txt", "analyze.py try/except import", "Partial", "MEDIUM", "Backend Developer", "OCR limits, install Tesseract", "Fails closed: paste text instead"),
    ("Browser", "Chrome Extension Manifest V3", "Popup, service worker, content script, settings, welcome", "extension/manifest.json and subfolders", "Implemented", "HIGH", "Extension Developer", "MV3, permissions, messaging", "Why MV3; host_permissions"),
    ("Browser", "chrome.storage (local/session)", "API URL, auto-scan, cache of scan results", "extension/utils/api.js, background/", "Implemented", "HIGH", "Extension Developer", "storage.local vs session", "Privacy of cached URLs"),
    ("Cybersecurity", "SSRF guard", "Blocks fetch to localhost, private IPs, cloud metadata", "backend/app/services/ssrf.py", "Implemented", "HIGH", "Security Lead", "SSRF, 169.254.169.254", "Why we must not fetch attacker URLs blindly"),
    ("Cybersecurity", "Parameterized SQL (? placeholders)", "All sqlite3 execute calls use bound parameters", "connection.py, routes/, reputation.py", "Implemented", "HIGH", "Backend / Security", "SQL injection", "How injection is prevented"),
    ("Cybersecurity", "bcrypt via passlib", "Password hashing at signup/login", "backend/app/services/auth.py, routes/user.py", "Implemented", "HIGH", "Backend / Security", "Hash vs encrypt, salt", "Why not store plaintext"),
    ("Cybersecurity", "JWT HS256 (python-jose)", "Bearer token; sub=username; login/signup pass 7-day expiry", "auth.py, user.py, ProtectedRoute", "Implemented", "HIGH", "Backend / Security", "JWT structure, Bearer header", "ACCESS_TOKEN_EXPIRE_MINUTES=30 unused"),
    ("Cybersecurity", "CORS allow_origins=['*']", "Dev convenience; credentials=True with * is weak", "backend/app/main.py", "Partial", "HIGH", "Security Lead", "CORS purpose", "Production must restrict origins"),
    ("Cybersecurity", "Report flood caps", "1 same URL / 24h and 20 reports / day per user", "backend/app/services/reputation.py", "Implemented", "MEDIUM", "Security Lead", "Abuse control", "Not a global API rate limit"),
    ("Cybersecurity", "Global API rate limiting", "No SlowAPI/middleware in main.py", "absent", "Planned", "MEDIUM", "Security Lead", "Rate limits", "Honest gap"),
    ("Cybersecurity", "Upload size cap 5 MB", "QR and screenshot endpoints", "backend/app/routes/analyze.py", "Implemented", "MEDIUM", "Backend / Security", "DoS via large files", "5_000_000 bytes"),
    ("APIs", "REST JSON (FastAPI)", "Primary integration for web + extension", "backend/app/main.py routers", "Implemented", "HIGH", "Backend / Frontend", "HTTP methods, JSON, status codes", "List important endpoints"),
    ("APIs", "Google Safe Browsing v4", "If GOOGLE_SAFE_BROWSING_API_KEY set in .env", "backend/app/services/external_intel.py", "Partial (key-dependent)", "HIGH", "Backend Developer", "Threat intel APIs, fail-soft", "What if key missing"),
    ("APIs", "OpenPhish feed", "Fallback feed when GSB key not used / as coded fallback", "external_intel.py OPENPHISH_FEED", "Implemented", "MEDIUM", "Backend Developer", "Feed caching", "Docstring also mentions URLhaus — verify code"),
    ("APIs", "RDAP / DNS", "Domain existence and approximate age", "external_intel.py", "Implemented", "MEDIUM", "Backend Developer", "DNS, RDAP, timeouts", "Offline behaviour"),
    ("AuthZ", "HTTPBearer optional vs required", "Scans public; reports require login; profile requires JWT", "analyze.py, reports.py, scans.py, user.py", "Implemented", "HIGH", "Backend Developer", "401 vs public scan", "Why scans can be anonymous"),
    ("Testing", "pytest", "API, psychology, URL, multi-channel tests", "backend/tests/*.py, requirements.txt", "Implemented", "HIGH", "All developers", "Arrange-act-assert, TestClient", "How you know it works"),
    ("Dev tools", "Git", "Version control of this repository", ".git, .gitignore", "Implemented", "HIGH", "All", "commit, branch, diff", "What is gitignored (.env, venv, db)"),
    ("Dev tools", "npm / Node", "Frontend install and Vite scripts", "frontend/package.json", "Implemented", "MEDIUM", "Frontend Developer", "npm install, run dev", "Lockfile purpose"),
    ("Deployment", "Local uvicorn + Vite only", "No Dockerfile, no compose, no cloud config in repo", "README.md", "Partial (local only)", "MEDIUM", "All", "Process model, ports 8000/5173", "How would you deploy v2"),
    ("Mobile", "Native Android SMS ingest", "Stub / planned path", "backend/app/services/mobile_source.py", "Planned", "LOW", "Extension / Mobile", "Why Chrome cannot read SMS", "Do not claim native SMS"),
]
...
def inventory_md_table() -> str:
    lines = [
        "| Domain | Technology | Actual use | Files / location | Status | Learning priority |",
        "|---|---|---|---|---|---|",
    ]
    for row in INVENTORY:
        cat, tech, purpose, files, status, pri, *_ = row
        lines.append(
            "| "
            + " | ".join(md_escape(x) for x in (cat, tech, purpose, files, status, pri))
            + " |"
        )
    return "\n".join(lines)


MARKDOWN = r'''# PHISHSHiELD AI
...
def write_markdown() -> None:
    text = MARKDOWN.replace("__INVENTORY_TABLE__", inventory_md_table())
...
def write_xlsx() -> None:
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Tech Stack Inventory"
    headers = [
        "Technology", "Category", "Purpose", "Files", "Status",
        "Priority", "Team Role", "Learning Topics", "Viva Topics",
    ]
    header_fill = PatternFill("solid", fgColor="0B1220")
    header_font = Font(bold=True, color="67E8F9", name="Calibri", size=11)
    fills = {
        "Implemented": PatternFill("solid", fgColor="D1FAE5"),
        "Partial": PatternFill("solid", fgColor="FEF3C7"),
        "Partial (key-dependent)": PatternFill("solid", fgColor="FEF3C7"),
        "Partial (weak)": PatternFill("solid", fgColor="FEF3C7"),
        "Partial (local only)": PatternFill("solid", fgColor="FEF3C7"),
        "Planned": PatternFill("solid", fgColor="DBEAFE"),
        "Not implemented": PatternFill("solid", fgColor="FECACA"),
        "Dependency present, usage not confirmed in live routes": PatternFill("solid", fgColor="E5E7EB"),
    }
    thin = Border(
    ...
    for r, row in enumerate(INVENTORY, 2):
        cat, tech, purpose, files, status, pri, role, learn, viva = row
        values = [tech, cat, purpose, files, status, pri, role, learn, viva]
        for c, val in enumerate(values, 1):
            ...
            if c == 5:
                cell.fill = fills.get(status, PatternFill("solid", fgColor="FFFFFF"))
            if c == 6 and pri == "HIGH":
                cell.font = Font(name="Calibri", size=10, bold=True, color="B91C1C")
    widths = [42, 18, 55, 48, 42, 12, 24, 40, 42]
    for i, w in enumerate(widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w
    ws.row_dimensions[1].height = 22
    ws.auto_filter.ref = f"A1:I{len(INVENTORY) + 1}"
    ws.freeze_panes = "A2"
    ...
    legend = [
        ("Implemented", "Live code path uses this technology"),
        ("Partial", "Optional, incomplete, or weakly configured"),
        ("Planned", "Stub or roadmap only — do not demo as done"),
        ("Not implemented", "Absent from the repository"),
        ("Dependency present, usage not confirmed", "In requirements or models/ but not the live sqlite3 route path"),
    ]
    for i, (a, b) in enumerate(legend, 2):
        ws2.cell(i, 1, a)
        ws2.cell(i, 2, b)
        ws2.cell(i, 1).fill = fills.get(a, PatternFill())
    ws2.column_dimensions["A"].width = 48
    ws2.column_dimensions["B"].width = 80

    ws3 = wb.create_sheet("Honesty rules")
    rules = [
        "Do not say Flask. Backend is FastAPI.",
        "Do not say live PostgreSQL. Database is SQLite backend/threats.db.",
        "Do not claim a trained CNN, sklearn model, or F1/accuracy numbers.",
        "model_confidence is signal agreement, not accuracy.",
        "model_probability equals the fused score, not a calibrated fraud probability.",
        "The fusion key 'ml' currently copies URL risk (content_analyzer.py).",
        "Psychology cannot alone produce HIGH/CRITICAL (cap 35).",
        "Extension does not read native SMS, passwords, OTP, or cookies.",
        "GET /api/v1/scans is public even if the UI is behind login.",
        "CORS allow_origins=['*'] is a known gap.",
        "SQLAlchemy models exist but live queries use sqlite3.",
        "Team member names were not in the repo — use roles, do not invent names.",
    ]
    ws3["A1"] = "Say this in the viva"
    ws3["A1"].font = header_font
    ws3["A1"].fill = header_fill
    for i, rule in enumerate(rules, 2):
        ws3.cell(i, 1, rule)
        ws3.cell(i, 1).alignment = wrap
...
def md_to_html(md: str) -> str:
    """Minimal Markdown → HTML for print (headings, tables, code, lists)."""
    import re
    ...
    def flush_table():
        ...
        for raw in table_buf:
            ...
            rows.append(cols)
        def _is_sep(row):
            return row and all(re.match(r"^:?-+:?$", (c or "").replace(" ", "")) for c in row)
    ...
    while i < len(lines):
        ...
        if line.startswith("```"):
            ...
            if not in_code:
                ...
                out.append(f"<pre{cls}><code>{html.escape(chr(10).join(code_buf))}</code></pre>")
                in_code = False
            i += 1
            continue
        if in_code:
        ...
        if line.startswith("# "):
            ...
            close_lists()
            out.append(f'<h2 id="s{len(out)}">{inline(line[3:].strip())}</h2>')
        elif line.startswith("### "):
            close_lists()
...
<head>
<meta charset="utf-8"/>
<title>PhishShield AI — Tech Stack &amp; Team Learning Guide</title>
<style>
  @page {
    size: A4;
    margin: 16mm 14mm 18mm 14mm;
    @bottom-center {
      content: "PhishShield AI  ·  Tech Stack & Team Learning Guide  ·  " counter(page);
  ...
  * { box-sizing: border-box; }
  ...
  .cover {
    ...
    min-height: 297mm;
    background: linear-gradient(165deg, #06101c 0%, #0b1f33 55%, #083344 100%);
    color: #e2e8f0;
  ...
  .cover .sub { font-size: 13pt; color: #cbd5e1; max-width: 150mm; }
  .cover .meta {
    margin-top: 28mm;
  ...
  h1 { font-size: 18pt; color: #0b1220; border-bottom: 2px solid #0e7490; padding-bottom: 6px; page-break-after: avoid; }
  h2 { font-size: 13.5pt; color: #083344; background: #0b1220; color: #67e8f9; padding: 7px 10px; margin-top: 18px; page-break-after: avoid; }
  h3 { font-size: 11.5pt; color: #155e75; margin-top: 14px; page-break-after: avoid; }
  ...
  code { font-family: Consolas, "Courier New", monospace; font-size: 8.6pt; background: #f1f5f9; padding: 0 3px; }
  pre {
    background: #0b1220;
    color: #e2e8f0;
    padding: 10px 12px;
  ...
  .table-wrap { width: 100%; overflow: visible; margin: 8px 0 12px; }
  table { width: 100%; border-collapse: collapse; table-layout: auto; font-size: 8pt; page-break-inside: auto; }
  ...
  .st-impl { background: #d1fae5 !important; font-weight: 600; color: #065f46; }
  .st-part { background: #fef3c7 !important; font-weight: 600; color: #92400e; }
  .st-plan { background: #dbeafe !important; font-weight: 600; color: #1e40af; }
  .st-no { background: #fecaca !important; font-weight: 600; color: #991b1b; }
  .warnbox {
  ...
  .footer-note { color: #64748b; font-size: 8.5pt; }
  strong { color: #0f172a; }
</style>
</head>
<body>
<div class="cover">
  <div class="kicker">TECHNICAL TRAINING LEAD · INTERNAL HANDBOOK</div>
  <p><span class="badge">CYBERSECURITY</span><span class="badge">SIH TRACK</span><span class="badge">CODE-TRUE</span></p>
  <h1>PHISHSHiELD AI</h1>
  <h1 style="font-size:20pt;margin-top:0">Tech Stack &amp; Team Learning Guide</h1>
  <p class="sub">Everything the team needs to learn to build, explain and defend the project — taken from the <em>current repository</em>, not from the idea note.</p>
  <div class="meta">
    <div>Running product name in code: <strong style="color:#fff">CyberGuard API 2.0</strong></div>
    <div>SIH product name: <strong style="color:#fff">PhishShield AI</strong></div>
    <div>Stack in one line: React 19 + Vite 8 + FastAPI + SQLite + Manifest V3 + weighted fusion (no trained CNN)</div>
    <div>Generated from repository inspection · 17 August 2026</div>
  </div>
</div>
__BODY__
</body>
...
def write_html() -> None:
    ...
    md = md.replace("# PHISHSHiELD AI\n# TECH STACK & TEAM LEARNING GUIDE\n", "", 1)
    body = md_to_html(md)
    replacements = [
        (">Implemented<", ' class="st-impl">Implemented<'),
        (">Partial<", ' class="st-part">Partial<'),
        (">Partial (key-dependent)<", ' class="st-part">Partial (key-dependent)<'),
        (">Partial (local only)<", ' class="st-part">Partial (local only)<'),
        (">Planned<", ' class="st-plan">Planned<'),
        (">Not implemented<", ' class="st-no">Not implemented<'),
    ]
    for old, new in replacements:
        body = body.replace(f"<td{old}", f"<td{new}")
    warn = (
        '<div class="warnbox"><strong>Honesty rule for every viva answer:</strong> '
        "FastAPI not Flask. SQLite not live PostgreSQL. Weighted fusion, not a trained CNN. "
        "Confidence is signal agreement, not accuracy. The extension does not read native SMS.</div>"
    )
    body = warn + body
    HTML_PATH.write_text(HTML_SHELL.replace("__BODY__", body), encoding="utf-8")
